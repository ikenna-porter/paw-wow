...
def get_all_messages(conversation_id: int, repo: MessageRepository = Depends()):
...
def create(message: MessageIn, repo: MessageRepository = Depends()) -> MessageOut:
    return repo.create(message)


@router.delete("/api/messages/{message_id}", response_model=bool)
def delete(message_id: int, repo: MessageRepository = Depends()) -> bool:
